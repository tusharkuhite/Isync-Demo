...
<div class="d-flex gap-3 top-input-space">
        <h5 class="space">
           System Email <span class="total"></span>
        </h5> 
        ...
        <div class="d-flex flex-wrap right-side-top">
            ...
            @if(isset($permission) && $permission != null && $permission->eWrite == "Yes")
            <div class="space">
                     <a href="{{route('admin.systemEmail.add')}}" class="btn btn-primary">+ Add</a>
            </div>
            @endif
        </div>
    </div>
...
<script type="text/javascript">
    ...
    function filter(vUniqueCode, vAction, vPages)
    {
        ...
        setTimeout(function() {
        $.ajax({
           url: "{{route('admin.systemEmail.ajax_listing')}}",
           type: "POST",
           headers:{'X-CSRF-TOKEN':$('meta[name="csrf-token"]').attr('content')},
           data: {vUniqueCode:vUniqueCode, vAction:vAction, pages:pages, vEmailTitle:vEmailTitle, vEmailCode:vEmailCode, eStatus_search:eStatus_search,eStatus:eStatus,eDeleted:eDeleted,},
