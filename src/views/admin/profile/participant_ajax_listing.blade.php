...
<tr>
   ...
   @endif
    ...
    @if (!empty($value->vWebpImage) && file_exists(public_path() . "/uploads/participant/participant_small/" . $value->vWebpImage))
     <a href="{{asset('uploads/participant/participant_small/'.$value->vWebpImage)}}" target="_blank">
     <img style="width: 90px;border-radius: 10px;" class="card-img-top" src="{{asset('uploads/participant/participant_small/'.$value->vWebpImage)}}" alt="Card image cap" /></a>
    @else
  ...
  @endif
   ...
   <td>
      ...
      <div class="bottom-btn text-center">
         ...
         <a href="{{url('admin/profile/participant_edit',$value->vUniqueCode)}}" data-toggle="tooltip_edit" title="Edit Data" data-id="{{$value->vUniqueCode}}" class="btn btn-outline-primary btn-sm me-1"><i class='bx bx-edit-alt pt-1 pb-1'></i></a>
         @endif
