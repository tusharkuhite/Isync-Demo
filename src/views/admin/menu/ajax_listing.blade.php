...
<tr>
   ...
   <td class="text-left text-nowrap">
      @if(!empty($value->vCode))
      <i class='{{$value->vCode}}'></i> - {{$value->vCode}}
   ...
   <td class="text-center text-nowrap">
    ...
    <td class="text-center">
      @if(!empty($value->eFeature) && !empty($value->eFeature))
   ...
   <td>
      ...
      <div class="bottom-btn text-center">
         ...
         <a href="{{url('admin/menu/edit',$value->vUniqueCode)}}" data-toggle="tooltip_edit" title="Edit Data" data-id="{{$value->vUniqueCode}}" class="btn btn-outline-primary btn-sm me-1"><i class='bx bx-edit-alt pt-1 pb-1'></i></a>
         @endif
...
@endif
<input type="hidden" id="Count" class="count" value="{{$MenuData}}">
