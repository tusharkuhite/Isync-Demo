...
<div class="d-flex gap-3 top-input-space">
        ...
        <div class="d-flex flex-wrap right-side-top">
            ...
            <div class="space">
                     <a href="{{route('admin.menu.add')}}" class="btn btn-primary">+ Add</a>
            </div>
            @endif
        </div>
    </div>
               ...
               <table class="table table-bordered">
                  <thead>
                     <tr>
                        ...
                        <th><input placeholder="Menu" id="vMenu" type="text" name="vMenu" class="search form-control"></th>
                        <th><input placeholder="Code" id="vCode" type="text" name="vCode" class="search form-control"></th>
                        <th><select id="iRoleId" name="iRoleId" class="form-select selectedrole">
                          ...
                          <th class="space-w"> 
                            <select class="form-select" name="eFeature" id="eFeature_search">
...
<script type="text/javascript">
    ...
    function filter(vUniqueCode, vAction, vPages,iRoleId)
    {
        ...
        setTimeout(function() {
        $.ajax({
           url: "{{route('admin.menu.ajax_listing')}}",
           type: "POST",
           headers:{'X-CSRF-TOKEN':$('meta[name="csrf-token"]').attr('content')},
           data: {vUniqueCode:vUniqueCode, vAction:vAction, pages:pages,vCode:vCode, vMenu:vMenu, iRoleId:iRoleId, eStatus_search:eStatus_search,eStatus:eStatus,eDeleted:eDeleted,eFeature_search:eFeature_search},
    ...
    $(document).on("change", "#eDeleted", function() {
        var iRoleId   = $('#iRoleId :selected').val();
        filter("", "delete", "",iRoleId);
    });
    
     $(document).on('click', '.ajax_page', function() {
        var vPages           = $(this).data("pages");
        var iRoleId          = $('#iRoleId :selected').val();
        filter("", "search", vPages,iRoleId);
